...
namespace CheckYourEligibility_FrontEnd.Attributes
{
    public class DobAttribute : ValidationAttribute
    {
        ...
        {
            var model = validationContext.ObjectInstance as ParentGuardian;
            ...
            {
                DateTime dob;
                var day = model.Day?.ToString().PadLeft(2, '0');
                var month = model.Month?.ToString().PadLeft(2, '0');
                var year = model.Year?.ToString().PadLeft(2, '0');
